# Liaoyk_code@qq.com

### 👋 Hi, I’m @Mrlyk

- 👀 **Coding 爱好者**，目前重心在 **AI Harness 工程** 与 **Agent 评测** 方向
- 🌱 热爱用代码解决实际问题，编写各种小工具
- 🛠️ 正在深入 AI 驱动的代码生成、自动化工程体系与智能开发工具
- 📍 目前在杭州工作
- 📫 **liaoyk_code@qq.com**

---

### 🚀 当前关注方向

- **AI Harness 工程**：构建高效、可靠的 AI Coding 开发流程与工程化体系
- **Agent 评测**：Agent 第一步～

---

- 🐎 **superharness**: https://github.com/Mrlyk/superharness 结合 superpowers 和 trellis 做的 AI Coding Harness 工具～欢迎体验 star
- 🔧 **jev-browser**: [https://github.com/Mrlyk/cc-plugins](https://github.com/Mrlyk/jev-browser) 基于决策模型 Jev 的浏览器操作工具
- 🧐 **Agent Skills**: https://github.com/Mrlyk/SKILLS 收藏的 SKILLS，强推 ask-me 和 grill-me

欢迎交流 AI Coding、AI Harness 以及任何有趣的 Coding 想法！
