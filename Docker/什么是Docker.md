...
> Docker 通过比较每一层的构建指令和输入文件的哈希值来确定是否需要重新构建：
>
> - 指令哈希：Docker 计算每条 RUN、COPY、ADD 等指令的哈希值。如果指令内容相同，哈希值相同。
> - 文件内容哈希：对于 COPY 和 ADD 指令，Docker 会计算被复制文件的内容哈希值。即使 Dockerfile 没变，文件内容改变了，哈希值就会改变。
> - 缓存失效：当某一层的哈希值与之前构建的缓存不匹配时，该层及之后的所有层都会重新构建
