...
配置打包开始的入口文件，webpack 会从这个入口开始递归的去查找依赖项。
...
**同时 entry 的 key（这里的 app）会作为 output 文件的 `[name]`，用来命名输出的包！**
...
`main` 字段指定了在用户 `require` 你的包时需要加载的入口文件的路径，以下面为例子
...
当用户 `require('my-package')` 时就会去引用`main` 字段声明的文件即 `dist/index.js`。

#### files

`files` 字段指定了哪些文件应该包含在发布的包中
...
如下配置就只会把 lib 文件夹和 types 文件下的 d.ts 文件发上去。
...
test 可以配置正则或 function 作为打包规则，其他属性均可继承splitChunks。
    ...
    splitChunks: {
      ...
      cacheGroups: { // 缓存组
        vendors: {
          ...
          chunks: 'initial' // initial 只对静态资源进行切割，也就是 require 和 import xx form 的，对 import() 这种动态导入的不会切割。如果设置为 all 则会对所有类型的资源进行切割。
        }
      }
    }
  }
