...
- item-1 = 0px + 400px / 4 * 2 = 200px；
- item-2 = 200px + 400px / 4 * 1 = 300px；
- item-3 =  0px + 400px / 4 * 1 = 100px
...
##### min-width/height 与弹性盒子

首先我们要知道2个点：

1. `min` 属性优先级高于普通属性高于`max`属性（在都是默认值的情况下），即`min-width` >  `max-width`  >`width` 
2. `min-width/height` 默认值` auto `用于弹性元素的默认最小宽度

知道这两点后，当我们**不给元素设置宽度时**，`min-width` 的优先级是最高的。而`min-widht` 的默认值是`auto`——表示最小内容宽度。这也是为什么在弹性盒子中，纵使我们允许盒子缩放`flex-shrink: 1` ，盒子依然会被内容撑开。

因为它的最小宽度就是元素宽度，不能在缩放了。

那么问题来了，我希望弹性盒子的子元素不要因为内容过长而撑大盒子要怎么做呢？很简单，设置`min-width/height` 的值为 0（或者任意小于 `width/height` 的值），这样 `widht/height` 就会重新用来规定元素的宽度，浏览器会根据父元素的及兄弟元素的宽度重新计算，不会撑开父元素了。
...
如果全都是文字元素，**比如全都是中文，那么每个字都会换行来实现最小宽度**，如果是英文则会以单词为单位换行。

- max-content：采用内部元素中**最大宽度**作为自身宽度

举个🌰：一个 box 内部图片 200px，文字 14px，20个文字不换行。那么 box 设置 `max-content` 他的宽度就有 280px，如果**父元素宽度不够会直接超出！**看着**像是设置了 white-space: no-wrap;** 的效果一样！
